...
class Sign:
    ...
    def __init__(self, img):
        ...
        self.findCnts()  # Find the contours of the sign
        self.findShape()
        self.findColours()
        self.findSign()
    ...
    def removeText(self):
        ...
        if np.array_equiv(no_text, np.zeros(self.grey_img.shape, np.uint8)):
            ...
            for i, h in enumerate(hier):
                cnt = contours[i]
                if cv2.contourArea(cnt) < 0.02 * self.area:
                    # Create a mask with the text
                    mask = np.zeros(self.grey_img.shape, np.uint8)
                    cv2.drawContours(mask, [cnt], 0, 255, -1)
                    # Add to the main mask
...
def stage1(sign="all", show=True):
    if sign == "all":
        ...
        name = sign.lower().replace(" ", "_")
        img = "stage_1\\" + name + ".jpg"
        sign = Sign(img)
        print(sign.category)
        print(sign.type)
        if show == True:
